...
public class XZGraphcolor {
    ...
    ArrayList<Integer> Tea_Name = null;
    ...
    public void setData(BasicDataInfo inputBasicData){
        ...
        List<ClassListInfo> classListInfo = inputBasicData.getClassListInfo();
        for(ClassListInfo classInfo:classListInfo)
        {
            Class.add(classInfo.getClassID());
        }
        ...
        List<TeaListInfo> teaListInfo = inputBasicData.getTeaListInfo();
        for(TeaListInfo teaInfo:teaListInfo)
        {
            Tea_Name.add(teaInfo.getTeaID());
        }
        ...
        if(Ctwo == 1)//触发优先排课
        {
            ...
            List<CtwoCrsListInfo> ctwoCrsListInfo = inputBasicData.getCtwoCrsListInfo();
            for(CtwoCrsListInfo ctwoCrsInfo:ctwoCrsListInfo)
            {
                for(j=0;j<CourseID.size();j++)
                {
                    if(CourseID.get(j) == ctwoCrsInfo.getCtwoCrsID())
                    {
                        crs_id = j + 1;
                        break;
                    }
        ...
        for(ClassCrsInfo classCrs:classCrsInfo)
        {
            ...
            {
                Class_Crs_Num[class_id][j+1] = course.getCourseNum();
                Class_Crs_Num_L[class_id][j+1] = course.getCourseLTNum();
                for(j=0;j<CourseID.size();j++)
                {
                ...
                else if(course.getCourseTeaNum() == 1)
                {
                    ...
                    {
                        if(Tea_Name.get(x) == course.getCourseTeaID())
                        {
                            tea_id = x + 1;
                            break;
                        }
        ...
        for(NoClassTimeInfo noClassTimeInfo:noClassTimeInfoList)
        {
            if(co.compare(noClassTimeInfo.getType(),"Class") == 0)//班级不排课
            {
                ...
                {
                    day = timeInfo.getDay();
                    List<AvbTimeInfo> avbTimeInfoList = timeInfo.getAvbTimeInfo();
                    k=0;
                    for(AvbTimeInfo avbTimeInfo:avbTimeInfoList)
                    {
                        k++;
                        Class_AvrT[class_id][day][k] = avbTimeInfo.getAvbTime();
                    }
                }
            ...
            else if(co.compare(noClassTimeInfo.getType(),"Teacher") == 0)//教师不排课
            {
                ...
                {
                    if(Tea_Name.get(j) == noClassTimeInfo.getId() )
                    {
                        tea_id = j + 1;//哪个教师
                        break;
                    }
                ...
                for(TimeInfo timeInfo:timeInfoList)
                {
                    ...
                    {
                        k++;
                        Tea_AvrT[tea_id][day][k] = avbTimeInfo.getAvbTime();
                    }
                }
            }
            else if(co.compare(noClassTimeInfo.getType(),"Course") == 0)
            {
                ...
                for(TimeInfo timeInfo:timeInfoList)
                {
                    ...
                    {
                        k++;
                        Crs_AvrT[crs_id][day][k] = avbTimeInfo.getAvbTime();
                    }
                }
            }
        }
        ...
        if(HBK_Num>0)
        {
            ...
            for(HBKInfo hbkInfo:hbkInfoList)
            {
                ...
                List<HBKClassInfo> hbkClassInfoList = hbkInfo.getHbkClassInfo();
                j = 0;
                for(HBKClassInfo hbkClassInfo:hbkClassInfoList)
                {
                    for(k=0;k<Class.size();k++)
                    {
                        if(Class.get(k) == hbkClassInfo.getHbkClassID())
                        {
                            class_id = k+1;//哪个班
                            break;
                        }
        ...
        for(YPKInfo ypkInfo:ypkInfoList)
        {
            ...
            for(YPKDataInfo ypkDataInfo:ypkDataInfoList)
            {
                ...
                List<YPKData> ypkDataList = ypkDataInfo.getYpkData();
                k = 0;
                for(YPKData ypkData:ypkDataList)
                {
                    k++;
                    crs_id = 0;//初始化
                    for(x=0;x<CourseID.size();x++)
                    {
                        if (CourseID.get(x) == ypkData.getYpkID())
                        {
                            crs_id = x + 1;//哪门课
                            break;
                        }
                    }
                    Class_Crs[class_id][day][k] = crs_id;
                }
            }
        }
    ...
    public void setData(String path){
        ...
        Class_Num = classData.getInt("classNum");
        JSONArray classList = classData.getJSONArray("classList");
        for(i = 0;i < classList.size();i++)
        {
            Class.add(classList.getInt(i));
        }
        JSONObject crsData = jsonObject.getJSONObject("crsData");
        Crs_Num = crsData.getInt("crsNum");
        JSONArray crsList = crsData.getJSONArray("crsList");
        for(i = 0;i < crsList.size();i++)
        {
            CourseID.add(crsList.getJSONObject(i).getInt("id"));
            //System.out.print(crsList.getJSONObject(i).getInt("id") + "\n");
            Coursename.add(crsList.getJSONObject(i).getString("name"));
        }
        JSONObject teaData = jsonObject.getJSONObject("teaData");
        Tea_Num = teaData.getInt("teaNum");
        JSONArray teaList = teaData.getJSONArray("teaList");
        for(i = 0;i < teaList.size();i++)
        {
            Tea_Name.add(teaList.getInt(i));
        }
        System.out.print("Crs_Num =" + Crs_Num + "\n");
        System.out.print("Class_Num =" + Class_Num + "\n");
        System.out.print("Tea_Num =" + Tea_Num + "\n");
        ...
        if(Ctwo == 1)//触发优先排课
        {
            JSONObject CtwoData = jsonObject.getJSONObject("ctwoData");
            JSONArray CtwoCrsArray = CtwoData.getJSONArray("ctwoCrs");
            for(i=0;i<CtwoCrsArray.size();i++)
            {
                for(j=0;j<CourseID.size();j++)
                {
                    if(CourseID.get(j) == CtwoCrsArray.getInt(i))
                    {
                        crs_id = j + 1;
                        break;
                    }
            ...
            C_Time = CtwoData.getInt("ctwoTime");
        }

        JSONArray classArray = jsonObject.getJSONArray("classCrsData");
        for(i = 0;i<classArray.size();i++)
        {
            ...
            JSONArray courseArray = classArray.getJSONObject(i).getJSONArray("course");
            for(j=0;j<courseArray.size();j++)
            {
                Class_Crs_Num[class_id][j+1] = courseArray.getJSONObject(j).getInt("courseNum");
                Class_Crs_Num_L[class_id][j+1] = courseArray.getJSONObject(j).getInt("courseLTNum");
                JSONArray teaArray = courseArray.getJSONObject(j).getJSONArray("teacherList");
                for(k=0;k<teaArray.size();k++)
                {
                    //System.out.print(teaArray.getJSONObject(k).getInt("id") + "\n");
                    for(x=0;x<Tea_Name.size();x++)
                    {
                        if(Tea_Name.get(x) == teaArray.getJSONObject(k).getInt("id"))
                        {
                            tea_id = x + 1;
                            break;
                        }
                    }
                    Class_Tea[class_id][j+1] = tea_id;
                    //System.out.print(Class_Tea[id][j+1]);
                }
            }
        }
        //////////////////////////////////////////////////////////////
        ...
        /////////////////////////////////////////////////////////////
        //不排课时间
        JSONArray noClassTimeArray = jsonObject.getJSONArray("noClassTime");
        for(i = 0;i<noClassTimeArray.size();i++)
        {
            ...
            if(co.compare(noClassTimeArray.getJSONObject(i).getString("type"),"Class") == 0)
            {
                ...
                JSONArray timeArray = noClassTimeArray.getJSONObject(i).getJSONArray("time");
                for(j=0;j<timeArray.size();j++)
                {
                    day = timeArray.getJSONObject(j).getInt("day");
                    JSONArray avbTimeArray = timeArray.getJSONObject(j).getJSONArray("avbTime");
                    for(k=0;k<avbTimeArray.size();k++)
                    {
                        Class_AvrT[class_id][day][k+1] = avbTimeArray.getInt(k);
                        //System.out.print(Class_AvrT[id][day][k+1] + " ");
                    }
                }
            }
            else if(co.compare(noClassTimeArray.getJSONObject(i).getString("type"),"Teacher") == 0)
            {
                for(j=0;j<Tea_Name.size();j++)
                {
                    if(Tea_Name.get(j) == noClassTimeArray.getJSONObject(i).getInt("id") )
                    {
                        tea_id = j + 1;//哪个教师
                    }
                }
        ...
        if(HBK_Num>0)
        {
            JSONArray HBKArray = jsonObject.getJSONArray("HBK");
            for(i=0;i<HBKArray.size();i++)
            {
                for(j=0;j<CourseID.size();j++)
                {
                    if(CourseID.get(j) == HBKArray.getJSONObject(i).getInt("HBKC") )
                    {
                        crs_id = j + 1;//哪门课程
                    }
                }
                ...
                JSONArray HBKClassArray = HBKArray.getJSONObject(i).getJSONArray("HBKClass");
        ...
        //预排课
        JSONArray YPKArray = jsonObject.getJSONArray("YPK");
        for(i=0;i<YPKArray.size();i++)
        {
            ...
            JSONArray timeArry = YPKArray.getJSONObject(i).getJSONArray("time");
            for(j=0;j<timeArry.size();j++) {
                day = timeArry.getJSONObject(j).getInt("day");
                JSONArray YPKTimeArray = timeArry.getJSONObject(j).getJSONArray("YPKTime");
	...
	*/
    ...
    private int Get_Initial_solution(int search_depth){
        int i;
        int num_best;
        int x, v;
        int iter;
    ...
    private void greed_ini_solution(){
        int i, j, k, l,v;
        int x,y;
        int num_best;
        int pick,day,crs;
    ...
    private void to_json(String path1, String path2){
    ...
    public ResultOutput getResultData(){
        int i,j,k;
        int x,y;
        y = 0;
            ...
            for (j = 1; j <= Weak_Day; j++) {
                for (k = 1; k <= Day_Crs; k++) {
                    ...
                    if(Class_Crs[i][j][k]>0)
                    {
                        ...
                        {
                            classResultList.setTeaID(Tea_Name.get(Class_Tea[i][Class_Crs[i][j][k]]-1));
                        }
                    }
        ...
        for (i = 1; i <= Tea_Num; i++) {
            for (j = 1; j <= Weak_Day; j++) {
                for (k = 1; k <= Day_Crs; k++) {
                    ...
                    teaResultList.setTeaID(Tea_Name.get(i-1));
                    teaResultList.setDay(j);
                    teaResultList.setCrs(k);
                    List<ClassIdList> classIdLists = new ArrayList();
                    if(Tea_Crs[i][j][k] > 0 && Tea_Crs[i][j][k] < 100)
                    {
                        ClassIdList classIdList = new ClassIdList();
                        classIdList.setClassID(Class.get(Class_Crs[Tea_Crs[i][j][k]][j][k]-1));
                        classIdLists.add(classIdList);
                    ...
                    else if(Tea_Crs[i][j][k] >= 100)
                    {
                        ...
                        for (x = 1; x <= Class_Num; x++)
                        {
                            ...
                            {
                                y = Tea_Crs[i][j][k];
                                classIdList.setClassID(Class.get(Tea_Crs[i][j][k] % 100 - 1));
                                classIdLists.add(classIdList);
                                Tea_Crs[i][j][k] = Tea_Crs[i][j][k] / 100;
                            }
                        }
